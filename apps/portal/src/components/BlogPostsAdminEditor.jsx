...
export default function BlogPostsAdminEditor() {
  ...
  const loadBlogConfig = async () => {
    if (!adminToken.trim()) {
      setLoading(false)
      setStatus({ type: 'error', message: 'Enter an admin token to load blog settings.' })
      return
    }
    ...
    try {
      ...
      const response = await fetch(`${serverUrl}/api/admin/blog-posts`, {
        headers: {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Unable to load blog posts configuration.')
      }

      setEditorValue(formatJson(data.config))
      setStatus({ type: 'success', message: 'Blog posts loaded successfully.' })
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.message || 'Unable to save blog posts configuration.')
      }

      setEditorValue(formatJson(data.config))
      setStatus({ type: 'success', message: 'Blog posts saved and published successfully.' })
    } catch (error) {
      setStatus({ type: 'error', message: error.message })
    } finally {
      setSaving(false)
  ...
  return (
    <section id="blog-admin" className="py-20 sm:py-24 bg-slate-900/30">
      <div className="max-w-4xl mx-auto px-3 sm:px-6">
        <div className="mb-8">
          <p className="text-blue-400 font-semibold uppercase tracking-widest mb-4">Blog Manager</p>
          <h2 className="text-2xl sm:text-4xl lg:text-5xl font-bold mb-3">Blog JSON Editor</h2>
          <p className="text-slate-400 text-sm sm:text-base max-w-3xl">
            Update blog/news posts from JSON and publish changes to the public newsroom.
          </p>
        </div>

        <div className="bg-slate-900 border border-slate-800 rounded-[28px] p-4 sm:p-6 lg:p-7 space-y-4">
          <div className="grid gap-2">
            <label htmlFor="adminBlogToken" className="text-sm text-slate-400">
              Admin Token
            </label>
            <input
              id="adminBlogToken"
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
            <p className="text-sm text-slate-400">
              File managed by backend: <span className="text-slate-300">news-posts.json</span>
            </p>
            <div className="flex gap-2">
              <button
                type="button"
                onClick={loadBlogConfig}
          ...
          <textarea
            ...
            spellCheck={false}
            disabled={loading}
            placeholder={loading ? 'Loading blog JSON...' : 'Blog JSON content'}
          />
